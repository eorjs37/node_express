...
async function fn_accounts(db){
    //await db.run('DROP TABLE IF EXISTS tbl_accounts');
    ...
    await db.run(
        ...
        (err) =>{
            if(!err){
                query = `INSERT OR IGNORE INTO tbl_accounts(id, email, password, grade, token) VALUES((SELECT id FROM tbl_accounts WHERE grade='owner'),'chleorjs37@gmail.com','vue','owner', null)`;
                db.run(query);
            }
            else {
                console.error(`tbl_accounts error : `, err);
            }
        }
    )
}
